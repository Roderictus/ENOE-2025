...
# Diccionario de variables a graficar con colores distintos
categorias = {
    'Sin Escolaridad': ('def_ing_mensual_ing_Sin_Escolaridad', '#7f8c8d'), # Gris
    'Primaria': ('def_ing_mensual_ing_Primaria', '#5ba0d0'),             # Azul claro
    'Secundaria': ('def_ing_mensual_ing_Secundaria', '#2878b5'),         # Azul medio
    'Preparatoria': ('def_ing_mensual_ing_Preparatoria', '#0f4c81'),     # Azul oscuro
    'Profesional (Universitaria)': ('def_ing_mensual_ing_Profesional', '#17202a'), # Casi negro
    'Posgrado': ('def_ing_mensual_ing_Posgrado', '#8e44ad')              # Morado para destacarlo
}
...
for label, (col, color) in categorias.items():
    if col in df.columns:
        ...
        ultimo_valor = df[col].dropna().iloc[-1]
